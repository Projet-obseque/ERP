...
import { refreshCompanyProfile } from "./tenant_context.js";

// --- INFOS AGENCE (fallback + profil dynamique) ---
...
async function chargerProfilSociete() {
    try {
        const remote = await refreshCompanyProfile();
        companyProfile = {
            ...DEFAULT_COMPANY_PROFILE,
            ...(remote || {})
        };
        const logoUrl = String(companyProfile.logo_url || "").trim();
...
window.refreshCompanyProfileInFacturation = async function() {
    await chargerProfilSociete();
    alert("Profil agence rechargé.");
};
...
window.generatePDFFromData = function(data, saveMode = false) {
    ...
    // Décale légèrement le bloc texte sous le logo pour éviter le chevauchement visuel.
    doc.text(String(companyProfile.company_name || DEFAULT_COMPANY_PROFILE.company_name), 15, 43);
    doc.setFontSize(9); doc.setFont("helvetica","normal"); doc.setTextColor(80);
    doc.text(String(companyProfile.address || DEFAULT_COMPANY_PROFILE.address), 15, 48);
    doc.text(`HABILITATION N° : 23-66-0205 | SIRET : ${String(companyProfile.siret || DEFAULT_COMPANY_PROFILE.siret)}`, 15, 52);
    doc.text(`N° TVA Intracommunautaire : FR92539270298 - RCS : ${String(companyProfile.rcs || DEFAULT_COMPANY_PROFILE.rcs)}`, 15, 56);
    ...
    doc.autoTable({
        ...
        body: body,
        theme: 'grid',
        styles: { fontSize: 8, cellPadding: 2, overflow: 'linebreak', textColor: [15, 15, 15] },
    ...
    // Affiche uniquement le montant TTC dans un cadre.
    const totalBoxX = 128;
