...
import { refreshCompanyProfile, saveCompanyProfile } from './tenant_context.js';
import { uploadDocumentToStorage } from './storage_manager.js';
...
function fillAgencySettingsForm(profile) {
    ...
    setValue('agency_rib_holder', profile?.rib_holder);
...
window.chargerParametresAgence = async function() {
    try {
        const profile = await refreshCompanyProfile();
        currentCompanyProfile = profile || {};
        fillAgencySettingsForm(currentCompanyProfile);
    } catch (err) {
        console.error(err);
        alert(`Impossible de charger les paramètres agence.\n\n${formatFirebaseError(err)}`);
    }
};

window.sauvegarderParametresAgence = async function() {
    const btn = document.getElementById('btn-save-agency-settings');
    ...
    try {
        ...
        const payload = {
            ...
            rib_holder: getValue('agency_rib_holder'),
            bank_name: getValue('agency_bank_name'),
            logo_url: getValue('agency_logo_url')
        };
...
window.uploadAgencyLogo = async function() {
    const input = document.getElementById('agency_logo_file');
    const logoUrlInput = document.getElementById('agency_logo_url');
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const logoUrlField = document.getElementById('agency_logo_url');
    if (logoUrlField) {
        logoUrlField.addEventListener('input', (e) => {
            updateAgencyLogoPreview(e.target.value);
        });
    }
});
...
window.showSection = function(id) {
    ...
    if (id === 'base') {
        DB.showBaseClientListView();
        DB.chargerBaseClients();
    }
    if (id === 'stock') DB.chargerStock();
    if (id === 'admin') {
        window.showAdminListView();
        DB.chargerDossiersAdminList();
        DB.chargerSelectImport();
    }
    if (id === 'settings') {
        window.chargerParametresAgence();
    }
};
